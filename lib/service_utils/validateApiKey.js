...
export async function validateApiKey(request) {
  ...
  if (!apiKey && request.url) {
    ...
    apiKey = searchParams.get('apiKey') || searchParams.get('api_key') || searchParams.get('key') || searchParams.get('x-api-key');
  }
  ...
  if (keyDoc && keyDoc.user) {
    // NOTE: We no longer increment usage here since that's handled in the withCorsProtection wrapper
    return keyDoc.user;
  }
